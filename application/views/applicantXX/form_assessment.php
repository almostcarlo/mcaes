<?php
    if($assessment_info){
        ...
        $details = $assessment_info[0]['details'];
        $position = $assessment_info[0]['position'];
        $nature_id = $assessment_info[0]['nature_id'];
   }else{
       $id = "";
       $details = "";
       $position = "";
...
<section class="panel">
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-sm-12">
                <h4 class="modal-title">General Assessment</h4>
            </div>
        </div>
...
<script>
	function save_assessment(){
		...
		if($('#textDetails').val() == ''){
			...
			$.post(base_url_js+'applicant/ajax_save_profile/assessment', $('#frm_facebox_assess').serialize(), function(data){
				if(data){
					if($('#textRecordId').val() == ''){
						$('#defaultNoticeContSuccess').html('Successfully added new assessment.');
					}else{
						$('#defaultNoticeContSuccess').html('Successfully updated assessment.');
					}
					...
					$('#defaultNoticeContError').html('Unable to save assessment.');
					$("#frm_ErrorNotice").removeClass("hidden");
				}
			});
		}
