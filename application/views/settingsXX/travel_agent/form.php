...
<section class="panel">
    <div class="panel-body">
        ...
        <?php echo form_open(BASE_URL.'settings/save/travel_agent', 'id="frm_facebox_travelagent"')?>
            ...
            <div class="row">
                <div class="col-md-8 mt-sm">
                    <div class="form-group">
                        ...
                        <input type="text" id="textContactP" name="textContactP" value="<?php echo $contact_person;?>" autocomplete="off" class="form-control input-sm" />
                    </div>
                </div>
                <div class="col-md-4 mt-sm">
                    <div class="form-group">
                        <label for=""> Contact No.</label>
                        <input type="text" id="textContactN" name="textContactN" value="<?php echo $contact_no;?>" autocomplete="off" class="form-control input-sm" />
                    </div>
                </div>
            </div>
...
<script>
    function save_this(){
        ...
        if($('#textName').val() == ''){
            $('#defaultNoticeContError').html('Agent Name is required.');
            $("#frm_ErrorNotice").removeClass("hidden");
            $('#textName').focus();
        }else if($('#textCode').val() == ''){
            $('#defaultNoticeContError').html('Agent Code is required.');
            $("#frm_ErrorNotice").removeClass("hidden");
            $('#textCode').focus();
        }else{
            $.post(base_url_js+'settings/save/travel_agent', $('#frm_facebox_travelagent').serialize(), function(data) {
                if(data){
                    ...
                    setTimeout(function(){
                        $.facebox.close();
                        window.location.replace(base_url_js+'settings/search/travel_agent');
                    }, 1000);
                }else{
                    $('#defaultNoticeContError').html('Unable to save travel agent.');
                    $("#frm_ErrorNotice").removeClass("hidden");
                }
            });
        }
