...
class Procurement_model extends CI_Model {
        ...
        public function add($what){
            ...
            switch($what){
                case 'requisition':
                    $max_req = $this->db->count_all_results('tbl_requesthd');
                    $req_no = "RQ".date("y").str_pad(($max_req+1),8,0,STR_PAD_LEFT);

                    $this->db->set('reqno', $req_no);
                    $this->db->set('reqdt', dateformat($_POST['textReqDate'],0));
                    $this->db->set('reqdept', $_POST['selectDept']);
                    $this->db->set('requestedby', $_SESSION['mcaes_user']['username']);
                    $this->db->set('remarks', $_POST['textRemarks']);
                    $this->db->set('createdby', $_SESSION['mcaes_user']['username']);
                    $this->db->set('createddt', 'NOW()', FALSE);
                    $this->db->insert('tbl_requesthd');

                    $desc = "New requisition has been created.";
                    $return_value = $req_no;
                break;
        ...
        public function edit($what){
            ...
            switch ($what) {
                case 'requisition':
                    ...
                    $this->db->where('reqno', $_POST['textReqNo']);
                    $this->db->update('tbl_requesthd');
                    $desc = "Requesition details has been updated.";
                    $id = $_POST['textReqNo'];
                    break;
                case 'req_item':
                    $this->db->set('itemdesc', $_POST['textDesc']);
